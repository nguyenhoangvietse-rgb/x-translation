...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (request.method === "GET" && chapterMatch) {
      ...
      if (!ch) {
        return new Response(
          `<div class="empty">Không tìm thấy chương ${id}.</div>`,
          {
            headers: { "Content-Type": "text/html; charset=utf-8" },
          },
        );
      ...
      if (content === null) {
        return new Response(
          `<div class="empty">Chương ${id} chưa được dịch.</div>`,
          {
            headers: { "Content-Type": "text/html; charset=utf-8" },
          },
        );
  ...
  ${renderAdminTable(books)}
    ...
    if (request.method === "POST" && processMatch) {
      ...
      return new Response(
        renderAdminRow({
          ...
          "<tr>",
          `<tr data-toast="Đã gửi yêu cầu chia chương '${escapeHtml(name)}'" data-toast-type="success">`,
        ),
        { headers: { "Content-Type": "text/html; charset=utf-8" } },
      );
    }
    ...
    if (request.method === "POST" && path.startsWith("/api/translate/")) {
      ...
      const badge = trigger.ok
        ? statusBadge("translating")
  ...
  <td style="padding:.7rem .8rem;border-bottom:1px solid #f0f0f0">
    ...
    if (request.method === "POST" && retranslateMatch) {
      ...
      const trigger = await triggerRetranslate(env, name, chapterId);
      if (!trigger.ok) {
        return new Response(trigger.error || "FAILED", { status: 500 });
      }
    ...
    if (request.method === "POST" && stopMatch) {
      ...
      return new Response(
        renderAdminRow(book).replace(
          "<tr>",
          `<tr data-toast="${toastMsg}" data-toast-type="success">`,
        ),
        { headers: { "Content-Type": "text/html; charset=utf-8" } },
      );
    }
    ...
    if (request.method === "POST" && coverUploadMatch) {
      ...
      if (!file) {
        return new Response(
          coverCell(name, false, "No file selected", "error"),
          { headers: { "Content-Type": "text/html" } },
        );
      }
      ...
      if (!allowedTypes.includes(file.type)) {
        return new Response(
          coverCell(name, false, "Only jpg/png/webp/gif allowed", "error"),
          { headers: { "Content-Type": "text/html" } },
        );
      }
      ...
      if (file.size > 2 * 1024 * 1024) {
        return new Response(
          coverCell(name, false, "Cover exceeds 2 MB limit", "error"),
          { headers: { "Content-Type": "text/html" } },
        );
      }
